...
GROUP_ID_TUJUAN = [
    "120363142428170648@g.us",
    "120363405739928770@g.us",
]
...
def nama_file_ss():
    return os.path.join(FOLDER_GAMBAR, f"Reporting_PS_{tanggal_hari_ini()}.png")

def nama_file_log():
    return os.path.join(FOLDER_LOG, "ps_log.txt")
...
async def ambil_screenshot():
    ...
    async with async_playwright() as p:
        ...
        target_url = f"{URL_PUBLISHED}?gid={GID_SHEET}&single=true&range={RENTANG_CELL}&widget=false&headers=false&chrome=false"
...
def kirim_via_whatsapp(path_gambar):
    ...
    try:
        ...
        caption_baru = (
            f"📊*Reporting Estimasi PS Indibiz Berjalan - SBT*\n"
        ...
        for group_id in GROUP_ID_TUJUAN:
            ...
            payload = {
                ...
                "file": {
                    "mimetype": "image/png",
                    "filename": f"Reporting_PS_{tanggal_hari_ini()}.png",
                    "data": encoded_string
                },
            ...
            try:
                ...
                catat_log(f"⚠️ Koneksi terputus ke {group_id} setelah request: {e}")
...
def tugas_harian():
    """Alur utama program."""
    catat_log("=" * 50)
    catat_log("🚀 Memulai tugas harian PS...")
    ...
    catat_log("🏁 Tugas harian PS selesai.")
    catat_log("=" * 50)

if __name__ == "__main__":
    print("=" * 55)
    print("  📱 Auto Screenshot & Kirim WA (PS)")
    print(f"  📌 Jumlah Grup Tujuan: {len(GROUP_ID_TUJUAN)}")
    print("=" * 55)
