...
ESKALASI_AKTIF = False  # Set True untuk mengaktifkan eskalasi
...
client = TelegramClient('sesi_mirror_pln', api_id, api_hash)
...
def nama_file_log():
    ...
    return os.path.join(FOLDER_LOG, "mirror_pln_log.txt")
...
def singkatan_distrik(distrik):
    """Mengubah nama distrik ke kode singkatan."""
    distrik_norm = normalisasi_distrik(distrik)
    return MAPPING_DISTRIK.get(distrik_norm, distrik_norm)
...
def ambil_pln_down_lebih_satu_jam(data_down=None, hostname_pemicu=None):
    sumber_data = data_pln_down if data_down is None else data_down
    hasil = []
    ada_pemicu = False
...
def buat_pesan_eskalasi(
    ...
    if daftar_down is None:
        daftar_down = ambil_pln_down_lebih_satu_jam()
    if not daftar_down:
        return None, []
    ...
    lines = ["🚨 *ESKALASI NE/PLN DOWN > 1 JAM*"]
    distrik_terdampak = []
    no = 1
...
def format_baris_down(no, info):
    """Format baris untuk laporan list WA (hanya kolom PLN)."""
    bagian = [nilai.strip() for nilai in str(info or "").split("|")]
    bagian += [""] * (5 - len(bagian))

    district = singkatan_distrik(bagian[0]) or "-"
...
def buat_laporan_list():
    teks_laporan = "*ALL NE/PLN DOWN - REG. Sumbagteng*\n"
    teks_laporan += (
        "DIST | HOSTNAME | DURASI DOWN | NodeB | IdPLN\n"
        f"{GARIS_TABEL}\n"
    )

    no = 1
    for hostname, info in data_pln_down.items():
        teks_laporan += format_baris_down(no, info) + "\n"
        no += 1
    ...
    no = 1
    for hostname, info in data_pln_up.items():
...
def simpan_ke_file_laporan(teks):
    ...
    nama_file = os.path.join(FOLDER_LOG, "pln_new_report.txt")
            ...
            for baris in baris_pesan:
                if 'PLN' in baris.upper() and '|' in baris and 'NE/PLN' not in baris.upper():
                    ...
                    if len(bagian) >= 1:
                        ...
                        # Pastikan hostname memang PLN (bukan baris header/judul)
                        if not hostname.startswith("PLN"):
                            continue
                        data_gabungan = f"{nama_distrik} | {baris.strip()}"
                        data_pln_down[hostname] = data_gabungan
                        ada_perubahan = True
                        hostname_terupdate.add(hostname)

                        if hostname in data_pln_up:
                            del data_pln_up[hostname]
            ...
            for baris in baris_pesan:
                if 'PLN' in baris.upper() and '| UP' in baris.upper() and 'UPLINK' not in baris.upper():
                    ...
                    if hostname:
                        ...
                        # Pastikan hostname memang PLN
                        if not hostname.startswith("PLN"):
                            continue
                        data_pln_up[hostname] = baris.strip()
                        ada_perubahan = True

                        if hostname in data_pln_down:
                            del data_pln_down[hostname]

        if ada_perubahan:
            simpan_log("🔔 Perubahan data PLN terdeteksi. Memperbarui laporan...")

            teks_laporan_baru = buat_laporan_list()
            simpan_ke_file_laporan(teks_laporan_baru)
            kirim_pesan_wa(teks_laporan_baru)

            # Eskalasi (OFF by default, set ESKALASI_AKTIF = True untuk mengaktifkan)
            if ESKALASI_AKTIF:
                daftar_eskalasi = ambil_pln_down_lebih_satu_jam(
...
async def main():
    ...
    await client.start()
    simpan_log(f"🚀 Program Mirror PLN Aktif! Mendengarkan {len(daftar_grup_target)} grup...")
